...
export function LancamentosEmpresariais() {
  ...
  const [itens, setItens] = useState<LancamentoEmpresarial[]>([]);
  const [contas, setContas] = useState<Conta[]>([]);
  const [cats, setCats] = useState<Categoria[]>([]);
  const [clientes, setClientes] = useState<PessoaCadastro[]>([]);
  const [forns, setForns] = useState<PessoaCadastro[]>([]);
  ...
  const carregar = async () => {
    ...
    try {
      ...
      const [r, a, c, cl, f] = await Promise.all([
        requisicao<LancamentoEmpresarial[]>(`/api/empresarial/lancamentos${q}`, {}, sessao.token),
        requisicao<Conta[]>("/api/empresarial/contas", {}, sessao.token),
        requisicao<Categoria[]>("/api/empresarial/categorias", {}, sessao.token),
        requisicao<PessoaCadastro[]>("/api/empresarial/clientes", {}, sessao.token),
      ...
      await requisicao(
        "/api/empresarial/lancamentos",
        {
          ...
          body: JSON.stringify({
            ...
            contaDestinoId: t === "TRANSFERENCIA" ? f.get("contaDestinoId") || null : null,
            categoriaId: f.get("categoriaId") || null,
            clienteId: f.get("clienteId") || null,
            fornecedorId: f.get("fornecedorId") || null,
  ...
  const cancelar = async () => {
    ...
    try {
      await requisicao(
        `/api/empresarial/lancamentos/${cancelando.id}`,
        { method: "DELETE" },
        sessao.token,
      );
      setCancelando(null);
  ...
  return (
    <AreaAutenticada tipo="empresarial">
      ...
      {erro ? (
        ...
        <p className="estado">Carregando lançamentos...</p>
      ) : itens.length === 0 ? (
        <p className="estado">Nenhum lançamento encontrado neste período.</p>
      ) : (
      ...
      {cancelando && (
        <Modal titulo="Cancelar lançamento" fechar={() => setCancelando(null)}>
          <ConfirmacaoAcao
            descricao="Esta ação irá cancelar o lançamento financeiro. O histórico será preservado."
