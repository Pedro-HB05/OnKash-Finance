...
export function MenuPerfil() {
  ...
  useEffect(() => {
    setEscuro(document.documentElement.dataset.theme === "escuro");
  ...
  const trocarTema = () => {
    ...
    setEscuro(proximo);
    localStorage.setItem("onkash.tema", proximo ? "escuro" : "claro");
      ...
      {aberto && (
        <div className="dropdown-perfil" role="menu">
          <div className="perfil-resumo">
            <span className="avatar grande">{iniciais(sessao.nome)}</span>
            <div>
              <strong>{sessao.nome}</strong>
              <small>{sessao.email}</small>
            </div>
          </div>
          <Link href="/perfil" role="menuitem" onClick={() => setAberto(false)}>
            <UserRound size={17} />
            Meu perfil
          </Link>
          ...
          <button role="menuitem" onClick={trocarTema}>
            ...
            {escuro ? "Usar modo claro" : "Usar modo escuro"}
          </button>
          <button
            className="sair-menu"
            role="menuitem"
